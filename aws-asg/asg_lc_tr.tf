...
resource "aws_launch_configuration" "launch_config" {
  count                       = var.create_before_destroy == "true" ? 1 : 0
  name_prefix                 = coalesce(var.launch_config_name, local.launch_config_name)
...
resource "aws_autoscaling_lifecycle_hook" "asg_lifecycle" {
  ...
  autoscaling_group_name  = var.create_before_destroy == "true" ? aws_autoscaling_group.asg_group[each.key].name : aws_autoscaling_group.asg_group_fl[each.key].name
